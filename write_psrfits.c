...
#else
    char *yuppi_dir = getenv("YUPPI_DIR");
    if (yuppi_dir==NULL) {
        fprintf(stderr, 
                "Error: YUPPI_DIR environment variable not set, exiting.\n");
        exit(1);
    }
    //sprintf(template_dir, "%s/src", yuppi_dir);
    strcpy(template_dir, yuppi_dir); // NEW
#endif
