...
usage(void)
{
  ...
  fprintf(stderr,"       -src: Source Name\n");
  fprintf(stderr,"             1 char* value\n");
  fprintf(stderr,"             default: `BLANK'\n");
  fprintf(stderr,"       -raj: Source RA  (J2000) in hhmmss.ss\n");
  fprintf(stderr,"             1 double value between 0 and 240000.00\n");
  fprintf(stderr,"             default: `0.00'\n");
  fprintf(stderr,"       -dej: Source Dec (J2000) in ddmmss.ss\n");
  fprintf(stderr,"             1 double value between -900000.00 and 900000.00\n");
  fprintf(stderr,"             default: `0.00'\n");
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-src", argv[i]) ) {
      int keep = i;
      cmd.src_nameP = 1;
      i = getStringOpt(argc, argv, i, &cmd.src_name, 1);
      cmd.src_nameC = i-keep;
      continue;
    }

    if( 0==strcmp("-raj", argv[i]) ) {
      int keep = i;
      cmd.rajP = 1;
      i = getDoubleOpt(argc, argv, i, &cmd.raj, 1);
      cmd.rajC = i-keep;
      checkDoubleLower("-raj", &cmd.raj, cmd.rajC, 240000.00);
      checkDoubleHigher("-raj", &cmd.raj, cmd.rajC, 0);
      continue;
    }

    if( 0==strcmp("-dej", argv[i]) ) {
      int keep = i;
      cmd.dejP = 1;
      i = getDoubleOpt(argc, argv, i, &cmd.dej, 1);
      cmd.dejC = i-keep;
      checkDoubleLower("-dej", &cmd.dej, cmd.dejC, 900000.00);
      checkDoubleHigher("-dej", &cmd.dej, cmd.dejC, -900000.00);
      continue;
    }
